...
dataset <- read_nhts_data("2009", csv_path = "C:/NHTS")
...
* [This File's Code](https://raw.githubusercontent.com/Westat-Transportation/summarizeNHTS/master/vignettes/summarizeNHTS-demo.R)
...
statistic <- make_table(
	...
	agg = "household_count",
	label = TRUE,
	factors = c("HHSIZE","HHVEHCNT")
)

make_html_table(statistic, digits = 0)
...
```{r}
make_bar_chart(statistic, squeeze_coord = "y", digits = 0)
```
...
make_bar_chart(statistic, order=T, digits=0)
...
```{r echo=F,warning=F}
nhts_2009$labels[nhts_2009$labels$NAME=="WHYTO"]$DESCRIPTION <- substr(nhts_2009$labels[nhts_2009$labels$NAME=="WHYTO"]$DESCRIPTION, 0, 22)
```
```{r}
...
statistic <- make_table(
	...
	agg_var = c("DWELTIME"),
	factors = c("WHYTO"),
	subset = "(!WHYTO %in% c(-9,-8,-7,-1,1,10,12))"
)
...
```{r echo=F, warning=F}
library(data.table)
dataset$data$trip[, STRTTIME := substring(sprintf("%04d", STRTTIME), 0, 2)]
```
...
make_bar_chart(statistic, digits=0)
...
statistic <- make_table(
	...
	factors = c("STRTTIME"),
	prop = TRUE,
	subset = "STRTTIME > 0 & WORKER == 1 & WKFTPT == 1"
)
...
statistic <- make_table(
	...
	agg = "person_trip_rate",
	factors = c("DRIVER"),
	subset = "DRIVER %in% c(1,2) & R_AGE > 18"
)
...
statistic <- make_table(
	data = dataset,
	agg = "person_trip_rate",
	factors = c("DRIVER", "WORKER"),
	subset = "DRIVER %in% c(1,2) & WORKER %in% c(1,2) & R_AGE > 18"
)
...
statistic <- make_table(
	...
	agg = "person_trip_rate",
	factors = c("USEPUBTR"),
	subset = "DRIVER %in% c(1,2) & R_AGE > 18 & USEPUBTR %in% c(1,2)"
)
...
statistic <- make_table(
	data = dataset,
	agg = "person_trip_rate",
	factors = c("TRAVDAY","WORKER"),
	subset = "WORKER %in% c(1,2) & R_AGE > 18"
)
...
statistic <- make_table(
	data = dataset,
	agg = "person_trip_rate",
	factors = c("TRAVDAY","PRMACT"),
	subset = "PRMACT %in% c(1,4,6)"
)
...
statistic <- make_table(
	...
	factors = c("HHSTATE"),
	label = FALSE,
	subset = "HHFAMINC %in% c(1,2,3,4,5)"
)
